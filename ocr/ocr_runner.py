...
class OCRRunner:
    ...
    def _split_image(self, img: np.ndarray, overlap: int = 100) -> List[Tuple[np.ndarray, int, int]]:
        ...
        将大图片分割成多个小块进行识别
        :param img: 原始图片
        :param overlap: 分块之间的重叠像素，防止文字被切断
        :return: [(分块图片, y偏移量, x偏移量), ...]
        """
        h, w = img.shape[:2]
    ...
    def _merge_ocr_results(self, blocks_results: List[Tuple[List, int, int]]) -> List:
        """
        合并所有分块的OCR结果，并去重
        :param blocks_results: [(ocr结果, y偏移量, x偏移量), ...]
        :return: 合并后的OCR结果
        """
        merged = []
    ...
    def locate_keyword(
        ...
        :param enable_split: 是否启用大图分块识别（默认启用）
        :param split_overlap: 分块重叠像素（默认100）
        :return: 匹配结果列表
        """
